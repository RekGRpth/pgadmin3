...
int pgServer::Connect(frmMain *form, bool askPassword, const wxString &pwd, bool forceStorePassword, bool askTunnelPassword)
{
	...
	if (!conn || conn->GetStatus() != PGCONN_OK)
	{
		...
		if (database.IsEmpty())
		{
			conn = new pgConn(host, service, hostaddr, DEFAULT_PG_DATABASE, username, password, iPort, rolename, ssl, 0, appearanceFactory->GetLongAppName() + " - Browser", sslcert, sslkey, sslrootcert, sslcrl, sslcompression);
			...
			{
				delete conn;
				conn = new pgConn(host, service, hostaddr, wxT("template1"), username, password, iPort, rolename, ssl, 0, appearanceFactory->GetLongAppName() + " - Browser", sslcert, sslkey, sslrootcert, sslcrl, sslcompression);
				if (conn && conn->GetStatus() == PGCONN_OK)
		...
		{
			conn = new pgConn(host, service, hostaddr, database, username, password, iPort, rolename, ssl, 0, appearanceFactory->GetLongAppName() + " - Browser", sslcert, sslkey, sslrootcert, sslcrl, sslcompression);
			if (!conn)
			{
				form->EndMsg(false);
